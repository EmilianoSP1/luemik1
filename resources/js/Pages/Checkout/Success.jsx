  ...
  return (
    <div className="relative min-h-screen bg-neutral-950 text-white grid place-items-center overflow-hidden px-6">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div
          className="absolute -top-40 -left-32 h-[540px] w-[540px] rounded-full blur-[90px] opacity-30"
          style={{ background: 'radial-gradient(closest-side, #10b981, transparent 65%)' }}
        />
        <div
          className="absolute -bottom-48 -right-40 h-[540px] w-[540px] rounded-full blur-[100px] opacity-25"
      ...
      <div className="max-w-lg w-full rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md p-8 text-center relative shadow-[0_20px_80px_-20px_rgba(0,0,0,.5)]">
        <FiCheckCircle className="mx-auto text-emerald-400 mb-3" size={48} />
        <h1 className="text-3xl font-extrabold mb-2">¡Pago exitoso!</h1>
        <p className="text-white/80 mb-2">Gracias por tu compra.</p>
        {session_id && <p className="text-xs text-white/50">Ref: {session_id}</p>}
        ...
        <p className="text-[11px] leading-5 text-white/55 mt-5">
          Te enviamos la confirmación a tu correo. Si necesitas ayuda con tu pedido, contáctanos.
        </p>
      </div>
    </div>
  );
